...
const CHCReport = () => {
  ...
  useEffect(() => {
    const fetchCombinedPatientData = async () => {
      ...
      const url = `https://shinova.in/_b_a_c_k_e_n_d/LIS/corporate_overall_report/?from_date=${formattedStartDate}&to_date=${formattedEndDate}`;
...
const fetchInvestigationFile = async (fileId) => {
  ...
  try {
    ...
    const result = await apiRequest(
      `https://shinova.in/_b_a_c_k_e_n_d/LIS/get_investigation_file/?file_id=${fileId}`,
      'GET'
    );
...
const handlePrint = async (patient, withLetterpad = true) => {
  try {
    ...
    const response = await apiRequest(
      `https://shinova.in/_b_a_c_k_e_n_d/LIS/corporate_health_report/?barcode=${patient.barcode}`,
      "GET"
    );
